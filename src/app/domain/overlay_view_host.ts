  ...
  getCalculatedOffset(offSetModel: OffSetModel) {
    let margin = 10;
        if (offSetModel.isCenterOverlay)
          return offSetModel.placement == 'bottom' ? { top: offSetModel.position.top + offSetModel.position.height + margin, left: offSetModel.position.left + (offSetModel.position.width / 2) - (offSetModel.overlayElementOffset.width / 2) } :
              offSetModel.placement == 'top' ? { top: (offSetModel.position.top) - (offSetModel.overlayElementOffset.height) - margin , left: offSetModel.position.left + ((offSetModel.mainElementOffset.width - offSetModel.overlayElementOffset.width) / 2) } :
              offSetModel.placement == 'left' ? { top: (offSetModel.position.top + (offSetModel.position.height / 2) - (offSetModel.overlayElementOffset.height / 2)), left: offSetModel.position.left - offSetModel.overlayElementOffset.width - margin } :
                { top: (offSetModel.position.top + (offSetModel.position.height / 2) - (offSetModel.overlayElementOffset.height / 2)), left: offSetModel.position.left + offSetModel.position.width + margin}
        else
...
export class BackDrop {
  ...
  private static overlayViewHost: OverlayViewHost;
  private static validationOverlayViewHost: OverlayViewHost;
